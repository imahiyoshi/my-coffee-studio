...
import { X, Sparkles, Loader2, MapPin, Tag, Coffee, Trash2 } from 'lucide-react';
...
最新の情報に基づき、以下のJSONフォーマットで出力してください。Markdownのコードブロック（\`\`\`json など）を含めず、純粋なJSONテキストのみを出力してください。
{
  ...
  // Initial prompt if none exist
  useEffect(() => {
    if (isOpen && !loading && recommendations.length === 0 && !generating && !errorMsg) {
      handleGenerate();
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-4 sm:p-6 transition-opacity">
      <div className="bg-stone-50 w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] animate-in slide-in-from-bottom-8 sm:slide-in-from-bottom-0 sm:fade-in sm:zoom-in-95 duration-200">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              >
                {generating ? <Loader2 className="w-3 h-3 animate-spin"/> : '別の豆を見る'}
              </button>
            </div>
            ...
            {generating ? (
              <div className="bg-white rounded-2xl p-8 border border-stone-100 flex flex-col items-center justify-center text-stone-400 shadow-sm">
                ...
                <p className="font-medium">検索中...</p>
              </div>
            ) : errorMsg ? (
              <div className="bg-white rounded-2xl p-8 border border-red-100 flex flex-col items-center justify-center text-red-500 shadow-sm text-center">
                <p className="font-medium mb-4 text-sm leading-relaxed">{errorMsg}</p>
                <button onClick={handleGenerate} className="px-4 py-2 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg font-bold transition-colors">
                  もう一度試す
                </button>
              </div>
            ) : recommendations[0] ? (
              <div className="bg-white rounded-2xl p-5 border-2 border-amber-100 shadow-sm relative overflow-hidden group">
                ...
                <div className="relative z-10 space-y-3">
                  <h4 className="text-lg font-bold text-stone-900 leading-tight">{recommendations[0].name}</h4>
                  <div className="flex flex-wrap gap-2 text-xs font-medium">
                    <span className="flex items-center gap-1 px-2 py-1 bg-stone-100 text-stone-600 rounded-lg">
                      <MapPin className="w-3 h-3" /> {recommendations[0].origin}
                    </span>
                    <span className="flex items-center gap-1 px-2 py-1 bg-stone-100 text-stone-600 rounded-lg">
                      <Tag className="w-3 h-3" /> {recommendations[0].estimatedPrice}
                    </span>
                  </div>
                  <p className="text-sm text-stone-600 leading-relaxed pt-2 border-t border-stone-100">{recommendations[0].taste}</p>
                </div>
              </div>
            ) : (
              <div className="bg-white rounded-2xl p-8 border border-stone-100 flex flex-col items-center justify-center text-stone-400 shadow-sm text-center">
                <p className="font-medium mb-4">まだ提案がありません</p>
                <button onClick={handleGenerate} className="px-4 py-2 bg-amber-50 hover:bg-amber-100 text-amber-600 rounded-lg font-bold transition-colors">
                  診断を開始する
                </button>
              </div>
            )}
