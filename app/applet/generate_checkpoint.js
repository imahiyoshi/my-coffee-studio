...
const IGNORE_EXTS = ['.png', '.jpg', '.jpeg', '.gif', '.ico', '.svg', '.woff', '.woff2', '.ttf', '.eot'];
const IGNORE_FILES = ['package-lock.json', 'checkpoint.json', 'generate_checkpoint.js'];
...
function getAllFiles(dirPath, arrayOfFiles) {
  ...
  files.forEach(function(file) {
    if (fs.statSync(dirPath + "/" + file).isDirectory()) {
      ...
      if (!IGNORE_FILES.includes(file) && !IGNORE_EXTS.includes(ext)) {
...
console.log("Successfully generated checkpoint.json with " + Object.keys(fileContents).length + " files.");
